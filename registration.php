...
<tr><td class="lefttd">Area</td><td><select name="s15" required><option value="">Select</option>
<?php
$cn=makeconnection();
$s="select * from area";
	$result=mysqli_query($cn,$s);
	...
	while($data=mysqli_fetch_array($result))
	{
		if(isset($_POST["show"])&& $data[0]==$_POST["s15"])
		{
			echo "<option value=$data[0] selected>$data[1]</option>";
		}
		else
...
<div class="footer">
		...
		<div class="copy">
			<p class="title">RaJ Ratan Redwan</p>
		</div>
	<div class="clear"></div>
</div>
</div>
...
//aloow certain file formats
	if($imageFileType != "jpg" && $imageFileType !="png" && $imageFileType !="jpeg" && $imageFileType !="gif"){
		...
		$cn=makeconnection();
			$s="insert into donarregistration(name,gender,age,mobile,b_id,a_id,email,pwd,pic) values('" . $_POST["t1"] ."','" . $_POST["r1"] . "','" . $_POST["t2"] . "','" . $_POST["t3"] . "','" . $_POST["t4"] . "','" . $_POST["s15"] . "','" . $_POST["t5"] . "','" . $_POST["t6"] .  "','" . basename($_FILES["t8"]["name"])  ."')";
