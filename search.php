...
<tr><td class="lefttd" style="padding-left:40px">Select Area </td><td><select name="s15" required><option value="">Select</option>

<?php
$cn=makeconnection();
$sa="select * from area";
	$result=mysqli_query($cn,$sa);
	$ra=mysqli_num_rows($result);
	//echo $r;
	while($data=mysqli_fetch_array($result))
	{
...
{
$cn=makeconnection();
$s="select * from donarregistration where bg_id='" .$_POST["s2"] ."' and area_id='" .$_POST["s15"] ."'";
	$result=mysqli_query($cn,$s);
	$r=mysqli_num_rows($result);
	//echo $r;
